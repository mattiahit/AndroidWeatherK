...
interface APIService {
    ...
    @GET("forecast")
    fun getForecastForCity(@Query("q") cityName: String, @Query("appid") appid: String): Single<ForecastResponse>

    @GET("weather")
    fun getWeatherForLocation(@Query("lat") lat: Double, @Query("lon") lon: Double, @Query("appid") appid: String): Single<WeatherResponse>

    @GET("forecast")
    fun getForecastForLocation(@Query("lat") lat: Double, @Query("lon") lon: Double, @Query("appid") appid: String): Single<ForecastResponse>
}